...
//
//
static void out_group_by(PQuery pq) {
...
static void out_group_by_lists(PQuery pq) {
    ...
    if (pq->group_by_output == GROUP_BY_OUTPUT_K_LISTS) {
        ...
        printf_f("\"ks\": [");
        for (int k = 0; k < pq->n_group_by_indexes; k++) {
            printf_f("%s {\"base\": %d, \"length\": %d} ",sep2,pq->group_by_distinct_base[k],pq->group_by_distinct_count[k]);
            sep2 = ",";
        }
        printf_f("],");
        ...
        if (pq->group_by_output == GROUP_BY_OUTPUT_V_LISTS) {
            ...
            printf_f("\"id\":%d, \"tp\":4,\"result\":{", pq->id);
                ...
                for (int k = 0; k < pq->n_group_by_indexes; k++) {
                    ...
                    int n, v;
                    sep2 = "";
                    for (v = pq->group_by_distinct_base[k], n=pq->group_by_distinct_count[k]; n; n--, v++ ) {
                        printf_f("%s%d",sep2,v);
                        sep2 = ",";
                    }
                    printf_f("]");
...
//    printf_f("\n");
    if (pq->schema_query) {
        ...
        out_no_group_by(pq);
    } else if (pq->group_by_output == GROUP_BY_OUTPUT_KV) {
        out_group_by(pq);
    } else {
