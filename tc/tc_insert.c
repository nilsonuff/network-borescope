 ...
 * 
 */
static int insert_into_tree(PSchema ps, PNode proot, int iroot, PAddress values, 
    ...
    for (inode=iroot, ichild=inode + 1, pf = ps->nfs+iroot, pf1=pf+1; ;inode++) {
        ...
        // Does a shared child already exist?
            ...
            } else if (pchild == pbase->nodes[ichild]) {
...
//    if (bottom) {
    if (bottom && !already_increased) { // 2019-02-13
        ...
        LOGIT_DECL(int d0);
        LOGIT_DECL(ps->tfs.get_content_data(pchild,&d0));
        ...
        LOGIT_DECL(int d);
        LOGIT_DECL(ps->tfs.get_content_data(pchild,&d));
    ...
    for (; inode >= iroot; inode--) {
        ...
        if (must_update_content) {
            if (icontent == ps->n_values) {
                ...
                insert_into_tree(ps, pcontent, icontent, values, ptree+1, ptree,po, 
                visited, increased);
            }
