...
*
*/
static int get_distinct_info(void *op, PQArgs pa, PByte params, int *distinct_count, int *distinct_base) {
    //printf("GEO: 1\n");
    if (op == op_zrect) {
        //printf("GEO: 2\n");
...
void register_class_geo(void) {
    ...
    c.to_address_types = strdup("DD"); // Double Double
    c.to_address_fmt = strdup("FF"); // Field Field
