...
*
*/
static void handle_group_by(char *name, PSchema ps, PQuery pq, PMetaData pmd) {
    int idx_value;
    int idx_slot;
    ...
    *err = '\0';
    ...
    if (group_by) {
        ...
        if (cJSON_IsString(group_by)) {
            handle_group_by(group_by->valuestring, ps, pq, pmd);
            ...
            handle_group_by(gb->valuestring, ps, pq, pmd);
            }
        } else {
    ...
    if (layout) {
        ...
        if (strncmp("kv",layout->valuestring,2)==0) {
            pq->group_by_output = GROUP_BY_OUTPUT_KV;
